...
import { Mail, Send, Inbox, RefreshCw, ChevronDown, ChevronUp, ExternalLink, Maximize2, PencilLine } from 'lucide-react';
...
export function FullEmailModal({ email, onClose }) {
  ...
  React.useEffect(() => {
    // Draft rows store a Gmail DRAFT id, not a message id — the message fetch
    // would fail, so fall back to the stored preview instead.
    if (email.is_draft) {
      setLoading(false);
      return;
    }
  ...
  }, [email.gmail_message_id, email.is_draft]);
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-3xl w-[95vw] max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-base font-semibold pr-6 flex items-center gap-2">
            {email.subject}
            {email.is_draft && (
              <Badge className="bg-amber-100 text-amber-700 border-0 text-xs flex-shrink-0">Draft — not sent</Badge>
            )}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-between items-start gap-3">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
              isDraft ? 'bg-amber-50' : email.direction === 'outbound' ? 'bg-blue-50' : 'bg-green-50'
            }`}>
              {isDraft
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                ...
                <Badge variant="outline" className={`text-xs flex-shrink-0 ${
                  ...
                  {isDraft ? 'In Gmail Drafts' : email.direction === 'outbound' ? 'Sent' : 'Received'}
                </Badge>
                {email.gmail_account && (
