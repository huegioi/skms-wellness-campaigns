  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input placeholder="e.g., Mental Health Awareness Month" value={form.name} onChange={e => set('name', e.target.value)} />
      </div>
      ...
      <div>
        ...
        <Input type="number" min={1} value={form.prep_trigger_days} onChange={e => set('prep_trigger_days', Number(e.target.value))} className="w-32" />
      </div>
      <div>
        <label className="text-sm font-medium text-gray-700 block mb-1">Notes (optional)</label>
        <Textarea rows={2} placeholder="Any additional context..." value={form.notes} onChange={e => set('notes', e.target.value)} />
      </div>
      ...
      <div className="flex gap-2 pt-1">
        <Button type="submit" disabled={saving} className="bg-[#013f7c] hover:bg-[#013f7c]/90 text-white">{saving ? 'Saving...' : 'Save Campaign'}</Button>
...
export default function CampaignCalendarTab() {
  ...
  const byMonth = MONTHS.map(month => ({ month, items: campaigns.filter(c => c.target_month === month) })).filter(g => g.items.length > 0);
    ...
    <div>
              ...
              <tbody>
                {campaigns.map((c) => (
                  <tr key={c.id} className={`border-b border-gray-50 hover:bg-gray-50/60 transition-colors ${!c.is_active ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3"><Badge className={`border text-xs font-medium ${CATEGORY_COLORS[c.category] || 'bg-gray-100 text-gray-600'}`}>{c.category}</Badge></td>
                    ...
                    <td className="px-4 py-3 text-center"><Switch checked={!!c.is_active} onCheckedChange={v => toggleActiveMutation.mutate({ id: c.id, is_active: v })} /></td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button size="icon" variant="ghost" className="w-7 h-7 text-gray-400 hover:text-[#013f7c]" onClick={() => openEdit(c)}><Pencil className="w-3.5 h-3.5" /></Button>
                        <Button size="icon" variant="ghost" className="w-7 h-7 text-gray-400 hover:text-red-500" onClick={() => { if (confirm(`Delete "${c.name}"?`)) deleteMutation.mutate(c.id); }}><Trash2 className="w-3.5 h-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <div key={c.id} className={`bg-white rounded-xl border border-gray-200 p-4 shadow-sm ${!c.is_active ? 'opacity-60' : ''}`}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="icon" variant="ghost" className="w-8 h-8 text-gray-400" onClick={() => openEdit(c)}><Pencil className="w-3.5 h-3.5" /></Button>
                    <Button size="icon" variant="ghost" className="w-8 h-8 text-gray-400" onClick={() => { if (confirm(`Delete "${c.name}"?`)) deleteMutation.mutate(c.id); }}><Trash2 className="w-3.5 h-3.5" /></Button>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <Badge className={`border text-xs font-medium ${CATEGORY_COLORS[c.category] || 'bg-gray-100 text-gray-600'}`}>{c.category}</Badge>
      ...
      <Dialog open={dialogOpen} onOpenChange={v => { setDialogOpen(v); if (!v) setEditing(null); }}>
        <DialogContent className="max-w-lg">
          ...
          <CampaignForm initial={editing || EMPTY_FORM} onSave={handleSave} onCancel={() => { setDialogOpen(false); setEditing(null); }} saving={saving} />
        </DialogContent>
      </Dialog>
    </div>
  );
