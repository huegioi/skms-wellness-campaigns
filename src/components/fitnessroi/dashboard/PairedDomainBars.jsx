  ...
  return (
    <div className="space-y-5">
      {DOMAINS.map(d => {
        ...
        const rows = [
          { label: 'You', val: leader, color: '#441D37' },
          { label: 'Team', val: team, color: '#441D37' },
        ];
        return (
          <div key={d.key}>
            <div className="flex justify-between items-baseline mb-2">
              <p className="text-sm font-medium text-mf-ink-2">{d.label}</p>
              {leader != null && team != null && (
                <p className="text-[10px] text-mf-ink-3">
                  You: {Math.round(leader)} · Team: {Math.round(team)}
                  <span className={`ml-1 font-semibold ${team < leader ? 'text-amber-600' : team > leader ? 'text-emerald-600' : 'text-mf-ink-3'}`}>
            ...
            <div className="space-y-1.5">
              {rows.map(row => (
                <div key={row.label} className="flex items-center gap-2">
                  <span className="text-xs text-mf-ink-3 w-10 shrink-0">{row.label}</span>
                  ...
                  <span className="text-xs font-semibold text-mf-ink-2 w-8 text-right">{row.val != null ? Math.round(row.val) : '—'}</span>
                </div>
              ))}
            </div>
