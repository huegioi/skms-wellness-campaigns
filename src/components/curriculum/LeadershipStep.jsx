...
export default function LeadershipStep({ selections, updateSelections, onNext, onBack }) {
  const leadership = Object.entries(productCatalog.coaching).filter(([key]) => 
    key.includes('leadership') || key.includes('Leadership')
  );
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-lg" style={{ color: '#666' }}>
          Elevate your leaders with specialized emotional intelligence training and coaching.
        </p>
      </div>
      ...
      <StepNavigation
        onNext={onNext}
        onBack={onBack}
        nextLabel="Continue to Coaching"
      />
    </div>
  );
}